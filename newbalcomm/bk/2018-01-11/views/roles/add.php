				...
				<div class="privilagepanel">
					<?php
						...
						if($prvPagesqlcnt>0)
						{
							...
							//echo '<pre>';print_r($prvPagedata);die;
									...
									<div class="col-sm-4">						    	
										<div class="panel panel-info">
											<div class="panel-heading">
												<h3 class="panel-title"><?php echo $value['pagename']; ?>													
													<?php
													$roleckbozes='';
													$roleischk=false;
													...
													if($prvOptPagescnt>0)
													{
														...
														foreach ($prvOptPagesdata as $key1 => $value1) {
															...
															$roleckbozes.= '<div class="col-sm-6"><label class="checkbox"><input type="checkbox" class="prvOptPageid chk'.$i.'" name="'.$chname.'" value="'.$value1['id'].'" '.$chname_ck.'><input type="hidden" class="prvOptPageids prvOptPageid_'.$i.'" name="prvOptPageid_'.$value1['id'].'" data-child="'.$i.'" value="'.$chname_val.'">&nbsp;<i class="fa '.$value1['icons'].'" aria-hidden="true"></i>&nbsp;&nbsp;'.$value1['optname'].'</label>
																<input type="hidden" name="prvPageid[]" value="'.$value1['id'].'" />
															</div>';
														}
													}
												?>
												<input id="selectbox<?php echo $i; ?>" alt="<?php echo $i; ?>" <?php if($roleischk==true) { /*echo 'checked="checked"';*/ } ?> type="checkbox" class="allcb" data-child="chk<?php echo $i; ?>">
												</h3>
