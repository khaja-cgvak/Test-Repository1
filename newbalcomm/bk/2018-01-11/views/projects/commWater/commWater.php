...
<!-- <input type="checkbox" id="toggle-two" <?php if(isset($sheetstatus[0]['status'])) { if(($sheetstatus[0]['status'])==1) { echo 'checked="checked"'; }  } else{ echo 'checked="checked"'; } ?> > -->
					...
					<?php 
						...
						echo $this->Common_model->displaySheetNum(PRJPRCLST,intval($sheetnumber),$masterprcid,'water');
					?></div>
				</div>
			</header>
			...
			the test data is a true record of the system performance achieved.
					...
					<div class="form-group clearfix">
						<div class="col-sm-4">
							...
							<input type="text" class="form-control" <?php if(isset($sidemenu_sub_enable)) { if(($sidemenu_sub_enable)==0) { echo 'readonly="true"'; }  } ?> name="commairwitness" placeholder="Enter Witness" value="<?php echo set_value('commairwitness', (isset($prcdata['u1fname'])) ? $prcdata['u1fname'].' '.$prcdata['u1lname'] : ''); ?>"  />
                			<?php echo form_error('commairwitness', '<div class="form-error">', '</div>'); ?>
                		</div>
						<div class="col-sm-4">
							<label>Company:<span class="mandatory"> *</span></label>
							<input type="text" class="form-control" <?php if(isset($sidemenu_sub_enable)) { if(($sidemenu_sub_enable)==0) { echo 'readonly="true"'; }  } ?> name="commaircmpy" placeholder="Enter Company" value="<?php echo set_value('commaircmpy', (isset($company_info['companyname'])) ? $company_info['companyname'] : ''); ?>"  />
					...
					<div class="form-group clearfix">
						<div class="col-sm-4">
							...
							<input type="text" class="form-control" <?php if(isset($sidemenu_sub_enable)) { if(($sidemenu_sub_enable)==0) { echo 'readonly="true"'; }  } ?> name="commairtestcomp" placeholder="Enter Test Completed By" value="<?php echo set_value('commairtestcomp', (isset($prcdata['u2fname'])) ? $prcdata['u2fname'].' '.$prcdata['u2lname'] : ''); ?>"  />
                			<?php echo form_error('commairtestcomp', '<div class="form-error">', '</div>'); ?>
                		</div>
						<div class="col-sm-4">
							<label>Services Contractor:<span class="mandatory"> *</span></label>
							<input type="text" class="form-control" <?php if(isset($sidemenu_sub_enable)) { if(($sidemenu_sub_enable)==0) { echo 'readonly="true"'; }  } ?> name="commairsercontract" placeholder="Enter Service Contractor" value="<?php echo set_value('commairsercontract', (isset($prcdata['servicecontractorname'])) ? $prcdata['servicecontractorname'] : ''); ?>"  />
