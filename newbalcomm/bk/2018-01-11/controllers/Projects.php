...
class Projects extends CI_Controller {
    ...
    {
        if($_FILES['timesheet_data']['tmp_name'] != ''){
        //$this->exitpage('projects/projview');  
        $my_json = file_get_contents($_FILES['timesheet_data']['tmp_name']);
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if($prcessid==0)
                {
                    ...
                    $ins_array1 = array(
                        ...
                        'comments'  =>  $my_json_new['commaircomments'][0],                        
                    ...
                    $timshtmaster_inc=array(
                        'projectprocesslistmasterid' => $prcessid,
                        'clientsign'=>$my_json_new['clientsign'][0],
                        'position'=>$my_json_new['position'][0],
                        'signdate'=>date(SQL_FORMAT,strtotime($my_json_new['signdate'][0])),
                        'createdon' => $created_date,
                        'createdby' => $this->session->userdata('userid')
                    );
                    ...
                    $procesSectionCat_pdata=$my_json_new['dayid[]'];
                    if(!empty($procesSectionCat_pdata))
                    {
                        foreach ($procesSectionCat_pdata as $psc_key => $psc_value) 
                        {
                            ...
                            if(!empty($my_json_new['daydate'.$psc_value][0]))
                            {
                                $daydate=date(SQL_FORMAT,strtotime($my_json_new['daydate'.$psc_value][0]));
                            }
                            if(!empty($my_json_new['dayhours'.$psc_value][0]))
                            {
                                $dayhours=date(SQL_FORMAT,strtotime($my_json_new['dayhours'.$psc_value][0]));
                            }
                            ...
                            $insarray_batch[] = array(
                              ...
                              'daydate'=>$daydate,
                              'dayhours'=>$dayhours,
                              'dayengg'=>implode(',', $my_json_new['dayengg'.$psc_value.'[]'])
                            );
                        }
                    }
                ...
                else
                {
                    ...
                    $timesheetid=intval($my_json_new['timesheetid']);

                    $timshtmaster_inc=array(
                        'clientsign'=>$my_json_new['clientsign'][0],
                        'position'=> $my_json_new['position'][0],
                        'signdate'=>date(SQL_FORMAT,strtotime($my_json_new['signdate']))
                    );
                    if($timesheetid==0)
                    {
                        ...
                        foreach ($procesSectionCat_pdata as $psc_key => $psc_value) 
                        {
                            ...
                            if(!empty($my_json_new['dayhours'.$psc_value][0]))
                            {
                                //$dayhours=date(SQL_FORMAT,strtotime($_POST['dayhours'.$psc_value]));
                                 $dayhours=($my_json_new['dayhours'.$psc_value][0]);
                            }
                            
                            if($my_json_new['tmeshtdataids'.$psc_value][0]==0)
                            {
                                $insarray_batch[] = array(
                                  'projectprocesslistmasterid' => $prcessid ,
                                  'timesheetid'=>$timesheetid,
        ...
        if( $my_json_new['dayid[]'])
        {
            $data['timesheetdata']=array();
            $value1=$my_json_new['dayid[]'];            
            foreach ($value1 as $key => $value) {
                $data['timesheetdata'][]=array(
                    'id'=>$my_json_new['tmeshtdataids'.$value][0],
                    'projectprocesslistmasterid'=>$prcessid,
                    'timesheetid'=>'',
                    'dayid' => $value,
                    'daydate'=>$my_json_new['daydate'.$value][0],
                    'dayhours'=>$my_json_new['dayhours'.$value][0],
                    'dayengg'=>implode(',',$my_json_new['dayengg'.$value.'[]'])
                );
            }

            $data['timshtmaster']=array(
                array(
                    'id'=>$my_json_new['timesheetid'][0],
                    'clientsign'=>$my_json_new['clientsign'][0],
                    'position'=>$my_json_new['position'][0],
                    'signdate'=>$my_json_new['signdate'][0]
                )
            );
    ...
    public function commAirFanPerRcd()
    {
       ...
       // }
        ...
        $data['sidemenu_sub_active'] = 'commAir';
        $data['sidemenu_sub_active1'] = $this->uri->segment(2);
        $data['sidemenu_sub_enable'] = $data['sheetstatus'][0];
        ...
        //$my_json = file_get_contents($_FILES['import']['tmp_name']);
        ...
        if ($this->input->post('submit'))
        {
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if($prcessid==0)
                {
                    ...
                    //$procesSection_pdata=$this->input->post('procesSection');
                    $procesSectionCat_pdata=$my_json_new['procesSection[]'];
                    ...
                    if(!empty($procesSectionCat_pdata))
                    {
                        ...
                        {                            
                            $processSecCatid=$psc_value;
                            $procesSection_pdata=$my_json_new['procesSection'.$psc_value.'[]'];
                            foreach ($procesSection_pdata as $ps_key => $ps_value) 
                            {
                                $staticcheck=(($my_json_new['commAirPreCommopt'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['commAirPreCommopt'.$processSecCatid.'_'.$ps_value][0]:'');
                                $comments=(($my_json_new['commAirPreCommcmd'.$processSecCatid.'_'.$ps_value][0]) ? $my_json_new['commAirPreCommcmd'.$processSecCatid.'_'.$ps_value][0]:'');
                                $testdata=(($my_json_new['commAirPreCommtestdata'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['commAirPreCommtestdata'.$processSecCatid.'_'.$ps_value][0]:'');
                                $testdata1=(($my_json_new['commAirPreCommtestdata1'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['commAirPreCommtestdata1'.$processSecCatid.'_'.$ps_value][0]:'');

                                $testdata_ans=(($my_json_new['testdata_ans'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['testdata_ans'.$processSecCatid.'_'.$ps_value][0]:'');
                                $testdata1_ans=(($my_json_new['testdata1_ans'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['testdata1_ans'.$processSecCatid.'_'.$ps_value][0]:'');                       
                ...
                else
                {
                    ...
                    $procesSectionCat_pdata=$my_json_new['processSecCatid[]'];
                    ...
                    if(!empty($procesSectionCat_pdata))
                    {
                        ...
                        {                            
                            ...
                            if(!empty($procesSection_pdata))
                            {
                                ...
                                $testdata_ans=(($my_json_new['testdata_ans'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['testdata_ans'.$processSecCatid.'_'.$ps_value][0]:'');
                                 $testdata1_ans=(($my_json_new['testdata1_ans'.$processSecCatid.'_'.$ps_value][0])?$my_json_new['testdata1_ans'.$processSecCatid.'_'.$ps_value][0]:''); 
                                 //echo $my_json_new['procesSectionId'.$ps_value][0];die;
                                 //echo '<pre>';print_r($my_json_new);die;
                    ...
                    if(!empty($upsarray_batch))
                    {
                        //echo '<pre>';print_r($upsarray_batch);die;
                        $this->db->update_batch(CAFANPERFTEST, $upsarray_batch,'id');
                       //echo $this->db->last_query();die;
