											...
											<ul class="gw-submenu processcarnavul" data-id="<?php echo $value['id']; ?>" id="processcarnavul_<?php echo $value['id']; ?>">
												<?php
													foreach ($processlist_data as $key1 => $value1) {	
														...
														if($value1['slug'])
														{
															$link2=site_url('projects/'.$value1['slug'].'/'.$proidnew);
														}
												?>
								          		<li data-id="<?php echo $value1['id']; ?>" data-name="Item <?php echo $value1['id']; ?>" id="item-<?php echo $value1['id']; ?>"> <a href="<?php echo $link2; ?>" class="<?php if($this->uri->segment(2)==$value1['slug']) { echo 'active_a';} ?>"><?php echo $value1['processtitle']; ?></a> </li>
								          		<?php
								          			}
								          		?>
								          	</ul>
