...
class Projects extends CI_Controller {
    ...
    public function addprojects()
    {
        ...
        {            
            $this->form_validation->set_rules('projectnumber', 'Project Number', 'trim|required');
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                $sql_query = "INSERT INTO project_sheet( projectid, sheetid, 
                                STATUS ) 
                                SELECT ".$projid." , id, isenabled
                                FROM  processlist";
        ...
        if ($this->input->post('submit'))
        {
            ...
            if(!empty($systemname_ck))
            {
                foreach($systemname_ck as $id => $data_ck)
                {
                    ...
                    $this->form_validation->set_rules('witnessedny[' . $id . ']', 'Witnessed By', 'required|trim');
                    $this->form_validation->set_rules('witnesseddate[' . $id . ']', 'Date', 'required|trim');
                    $this->form_validation->set_rules('testcmpby[' . $id . ']', 'Engineer', 'required|trim');
                    $this->form_validation->set_rules('testcmpdate[' . $id . ']', 'Date', 'required|trim');
                }
            }
    ...
    public function getallProjects()
        ...
        $columns = array(
            //array( 'db' => 'p.projectnumber', 'dt' => 0, 'field' => 'projectnumber'),  
            array( 'db' => 'p.projectnumber', 'dt' => 0, 'field' => 'projectnumber','formatter'=>function($d,$row){
                $newpid=$this->Common_model->encodeid($row['id']);
                return '<a href="'.site_url('projects/projview/'.$newpid).'">'.$d.'</a>';
            }),          
            ...
            array( 'db' => 'cn.contactfirstname',  'dt' => 2, 'field' => 'contactfirstname'),
            array( 'db' => 'cn.contactlastname',  'dt' => 2, 'field' => 'contactlastname','formatter'=> function($d,$row){ return $row['contactfirstname'].' '.$row['contactlastname'];}),
            ...
            array( 'db' => 'p.projectdescription',  'dt' => 4, 'field' => 'projectdescription'),
            
            //array( 'db' => 'p.projectstatus',  'dt' => 5, 'field' => 'projectstatus'),
            array( 'db' => 'p.isactive',   'dt' => 5, 'field' => 'isactive','formatter' => function( $d, $row ) {
                    return (($d==1)?'Active':'Inactive');
                }),
            array( 'db' => 'p.projectstartdate',  'dt' => 6, 'field' => 'projectstartdate','formatter' => function( $d, $row ) { return date('d M Y',strtotime($d));}),
    ...
    public function projview()
    {
        ...
        $this->session->set_userdata('system_value', $system_id);
        //$data['sidemenu'] = 'hide';
        $this->load->view('common/header',$data);
    ...
    {
        $this->exitpage('projects/projview');
        $checked_data = $this->check_is_offline_data($_POST);
        $_POST = $checked_data;
        $proidnew = $this->uri->segment(3);
        $proid = intval($this->Common_model->decodeid($proidnew));        
        $data['proid'] = $proid;
        ...
        $latestid=$this->uri->segment(4);
        if(!empty($latestid)&& $latestid=='latest')
        {
            $getlatestsheetid=$this->MProject->getlatestsheetid($masterprcid,'');
            if($getlatestsheetid!=false)
            {
                $path = base_url('projects/'.$this->uri->segment(2).'/'.$proidnew.'/'.$getlatestsheetid);
                redirect($path);
            }
            else
            {
        ...
        $data1 =$this->MProject->getProcessTitle($masterprcid);  
        $data['title'] = $data1[0]['processtitle'];
        //$data['sheetstatus'] = $data1[0]['isenabled'];        
           ...
           // $this->form_validation->set_rules('reportref', 'Ref', 'trim|required');
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if($prcessid==0)
                {
                    ...
                    $this->db->Where('id', $this->input->post('json_file_id'));
                    $this->db->update(OFFLINE, array('is_imported'=>'1'));
        ...
        $data['sidemenu_sub_enable'] = $data['sheetstatus'][0]['status'];
        //print_r($data['sidemenu_sub_enable']); die;       
        $data['userall']=$this->MProject->getUsers(0);
        $this->load->view('common/header',$data);
    ...
    public function timeSheet()
    {
        ...
        if ($this->input->post('submit'))
        {
            ...
            if ($this->form_validation->run() == TRUE)
            {
                    ...
                    if(!empty($procesSectionCat_pdata))
                    {
                        foreach ($procesSectionCat_pdata as $psc_key => $psc_value) 
                        {
                            ...
                            $insarray_batch[] = array(
                              ...
                              'dayid' => $psc_value,
                              'daydate'=>$daydate,
                              'dayhours'=> $_POST['dayhours'.$psc_value],
                              'dayengg'=>implode(',', $_POST['dayengg'.$psc_value])
                            );
                        }
                    }
                ...
                else
                {
                    ...
                    //echo '<pre>';print_r($timshtmaster_inc);die;
       ...
       // echo '<pre>';print_r($timshtmaster->result_array());die;
    ...
    public function waterTreatmentSysWitCer()
    {
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if($prcessid==0)
                {
                    ...
                    $insert_id=$this->db->insert_id();
                     ...
                     $this->db->Where('id', $this->input->post('json_file_id'));
                     $this->db->update(OFFLINE, array('is_imported'=>'1'));
                     $watertreat_syscert_slected_engineer = $this->input->post('selected_engineer_id');
                     $this->session->set_userdata('watertreat_syscert_slected_engineer', $watertreat_syscert_slected_engineer);
                    $this->session->set_flashdata('project_message', 'New '.$data['title'].' has been created successfully...');
                }
                else
                {
                    ...
                    if($this->input->post('selected_engineer_id')!='')
                    {
                        $this->session->set_userdata('watertreat_syscert_slected_engineer',$this->input->post('selected_engineer_id')); 
                    }
                    $this->session->set_flashdata('project_message', $data['title'].' has been updated successfully...');
        ...
        if($prcessid!=0)        
        {
            ...
            $data['prcdata']=$prcdatas[0];
           // echo '<pre>';print_r($data['prcdata']);die;
        }
        $data['prcessid']=$prcessid;
        $data['sheetstatus']=$this->MProject->getPrjSheetStatus($proid,$masterprcid);  
        $data['custom_side_menu'] = 'projprsmenu';
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if($prcessid==0)
                {
                    ...
                    $insert_id=$this->db->insert_id();
                     ...
                     $this->db->Where('id', $this->input->post('json_file_id'));
                     $this->db->update(OFFLINE, array('is_imported'=>'1'));
                     $commWater_syscert_slected_engineer = $this->input->post('selected_engineer_id');
                    $this->session->set_userdata('commWater_syscert_slected_engineer', $commWater_syscert_slected_engineer);
                    $this->session->set_flashdata('project_message', 'New '.$data['title'].' has been created successfully...');
                }
                else
                {
                    ...
                    if($this->input->post('selected_engineer_id')!='')
                    {
                        $this->session->set_userdata('commWater_syscert_slected_engineer',$this->input->post('selected_engineer_id')); 
                    }
        ...
        if ($this->input->post('submit'))
        {
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if($prcessid==0)
                {
                    ...
                    $this->db->Where('id', $this->input->post('json_file_id'));
                    $this->db->update(OFFLINE, array('is_imported'=>'1'));
                    $commAir_syscert_slected_engineer = $this->input->post('selected_engineer_id');
                    $this->session->set_userdata('commAir_syscert_slected_engineer', $commAir_syscert_slected_engineer);
                    //echo $this->session->userdata('commAir_syscert_slected_engineer');die;
                    $this->session->set_flashdata('project_message', 'New '.$data['title'].' has been created successfully...');
                }
                else
                {
                    ...
                    if($this->input->post('selected_engineer_id')!='')
                    {
                        $this->session->set_userdata('commAir_syscert_slected_engineer',$this->input->post('selected_engineer_id')); 
                    }
                   // echo $this->session->userdata('commAir_syscert_slected_engineer');die;     
                    $this->session->set_flashdata('project_message', $data['title'].' has been updated successfully...');
    ...
    {    
       ...
       if($this->uri->segment(5) != '') {
            ...
            $data['sheetstatus'] = array(array('status'=> $this->uri->segment(5)));
        }
        else 
        {
            $data['sheetstatus']=$this->MProject->getPrjSheetStatus($proid,$masterprcid);
        ...
        $data['sidemenu_sub_active'] = 'commAir';
        $data['sidemenu_sub_active1'] = $this->uri->segment(2);
        $data['sidemenu_sub_enable'] = base64_decode(urldecode($this->uri->segment(4)));
        //echo '<pre>';print_r($data['sidemenu_sub_enable']);die;
        $this->load->view('common/header',$data);
        ...
        //echo $_FILES['import']['tmp_name'].'hai';die;
        
        //$my_json = file_get_contents($_FILES['json_data']['tmp_name']);

        /*********json data*********/


        //$my_json_new = json_decode($my_json, true);
        //$my_json_new = array_column($my_json_new, 'value', 'name');
      //echo '<pre>';print_r($my_json_new);die;
            ...
            //{
                ...
                if($prcessid==0)
                {
                    $ins_array1 = array(
                        ...
                        'processid' => $masterprcid,
                        'sheetnumber' => 0,                        
                        'system' => $my_json_new['projsystem'][0],                    
                        'referenceno' => $my_json_new['reportref'][0],
                        'comments'  =>  '',                        
                        'engineerid' => $my_json_new['commairreptenggsign'][0],
                        'reportdate' => date('Y-m-d H:i:s',strtotime( $my_json_new['commairreptenggdate'][0])),
                        'createdon' => $created_date,
                        'createdby' => $this->session->userdata('userid')
                    );
                    ...
                    $ins_array = array(
                        'projectprocesslistmasterid' => $prcessid,
                        'reportdescription' => $my_json_new['commairreptcomments'][0],
                        'createdon' => $created_date,
                        'createdby' => $this->session->userdata('userid')
                    );
                ...
                $sheetstatus= $my_json_new['shtenadis'][0];
    ...
    public function commAirFanPerRcd()
    {
        ...
        //echo '<pre>';print_r($data['prcdata']);die;
        //if($this->uri->segment(5) !='') {
            //echo $this->uri->segment(5).'hai';die;
       ...
       // }
        ...
        $data['sidemenu_sub_active1'] = $this->uri->segment(2);
        $data['sidemenu_sub_enable'] = $data['sheetstatus'][0]['status'];
        //echo '<pre>';print_r($data['sheetstatus']); die;
        $this->load->view('common/header',$data);
    ...
    public function commAirGrillBalRcd()
    {
        ...
        $data['sidemenu_sub_active'] = 'commAir';
        $data['sidemenu_sub_enable'] = $data['sheetstatus'][0]['status'];
        //$data['sidemenu_sub_enable'] = $this->uri->segment(4); 
        //echo '<pre>';print_r($data['sidemenu_sub_enable']); die;
        $this->load->view('common/header',$data);
    ...
    public function commAirDirGrillRcd()
    {
            ...
            $data1 =$this->MProject->getProcessTitle($data['masterprcid']);  
            $data['title'] = $data1[0]['processtitle'];
            //$data['sheetstatus'] = $data1[0]['isenabled'];
            //$data['sidemenu'] = 'hide';
    ...
    public function commAirVlmControl()
    {
        ...
        //echo $proid.'//'.$prcessid.'//'.$masterprcid;die;
        $data1 =$this->MProject->getProcessTitle($masterprcid);  
        $data['title'] = $data1[0]['processtitle'];
        //$data['sheetstatus'] = $data1[0]['isenabled'];
    ...
    public function check_is_offline_data($posted_data)
    {
        if(isset($posted_data['hidden_json_data']) && !empty($posted_data['hidden_json_data'])){
            $json_data = json_decode($_POST['hidden_json_data'], true);
                    $posted_data = $json_data;
                    if($posted_data['process_slug'] == 'timeSheet'){
                        $posted_data = $this->setEngineersIDsInArr($posted_data);
                    }
                    return $posted_data;
        }
        return $posted_data;
    ...
    /**
     * function to replace names of engineers entered in offline 
     * -with ids group by days in array for each day 
     */
    public function setEngineersIDsInArr( $posted_data) {

        $project_id_encoded = $this->uri->segment(3);
        $project_id = intval($this->Common_model->decodeid($project_id_encoded));
    
        for($i=1;$i<=7;$i++){
            /**Dynamic Array Naming as $engineerNamesArr1,$engineerNamesArr2,3...*/
            ${"engineerNamesArr$i"} = explode(",", $posted_data['dayengg'.$i][0]);
        }
        for($i =1; $i<=7; $i++)
        {
            ${"engIdArr$i"} = array();
            foreach(${"engineerNamesArr$i"} as $engName)
            {
                 
                $firstLastNameArr = explode(" ", $engName);
                $engineer_id = $this->Common_model->getProjectEngineerId($firstLastNameArr, $project_id)->userid;
                if(!empty($engineer_id)){
                    ${"engIdArr$i"}[] = $engineer_id;
                }
                else{
                    $this->session->set_flashdata('success_message', '<h5 style="color:red;">Invalid Engineer Names entered in offline sheet</h5>');
        ...
        for($i=1;$i<=7;$i++){
            $posted_data['dayengg'.$i] = ${"engIdArr$i"};
        }
